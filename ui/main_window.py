...
from utils.metrics import calculate_psnr, calculate_ssim, calculate_ber_text
...
class MainWindow(QMainWindow):
    ...
    def _create_file_operation_group(self):
        ...
        # 导入含密图像
        self.btn_load_stego=QPushButton("导入含密图像")
        self.lbl_stego_path = QLabel("未选择文件")
        self.lbl_stego_path.setWordWrap(True)
        ...
        layout.addWidget(self.btn_load_stego, 2, 1)
        layout.addWidget(self.lbl_stego_path, 3, 0, 1, 2)

        layout.addWidget(self.btn_save_stego, 2, 2)
        layout.addWidget(QLabel("导出格式:"), 5, 0)
        layout.addWidget(self.cmb_export_format, 5, 1)
        layout.addWidget(self.btn_export_data, 6, 0, 1, 3)
    ...
    def _connect_signals_slots(self):
        ...
        self.btn_load_stego.clicked.connect(self._on_load_stego_clicked)
    ...
    def _on_load_cover_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"无法加载图像: {str(e)}。请检查文件路径和文件格式是否正确。")
            self.status_label.setText("载体图像加载失败")
            return

    def _on_load_stego_clicked(self):
        """加载含密图像"""
        self.status_label.setText("选择含密图像...")
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "选择含密图像",
            "",
        ...
        if not file_path:
            self.status_label.setText("已取消选择含密图像")
            return
        #询问用户是否需要输入原始秘密信息以计算攻击后的BER？
        reply=QMessageBox.question(
            self,
        "提示",
        "是否需要输入原始秘密信息以计算攻击后的BER？",
        ...
        if reply == QMessageBox.Yes:
            ...
            if ok and secret:
                self.original_secret=secret
            else:
                self.lbl_ber.setText("BER：未提供原始信息")
        try:
            from utils.image_utils import read_image
            self.stego_image = read_image(file_path)
            self.lbl_stego_path.setText(file_path)
            self.stego_viewer.set_image(self.stego_image)
            # 启用提取按钮
            self.btn_extract.setEnabled(True)
            self.btn_apply_attack.setEnabled(True)
            self.status_label.setText("含密图像加载成功")
        except Exception as e:
            QMessageBox.critical(self, "错误", f"无法加载含密图像: {str(e)}")
            self.status_label.setText("含密图像加载失败")
            return
    ...
    def _on_save_stego_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"保存图像失败: {str(e)}。请检查保存路径是否有写入权限。")
    ...
    def _on_export_data_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"导出数据失败: {str(e)}。请检查保存路径是否有写入权限。")
    ...
    def _on_load_secret_file_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"加载秘密信息文件失败: {str(e)}。请检查文件路径和文件编码是否正确。")
    ...
    def _on_embed_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"嵌入信息失败: {str(e)}。请检查嵌入算法和输入数据是否正确。")
    ...
    def _on_extract_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "错误", f"提取信息失败: {str(e)}。请检查提取算法和输入数据是否正确。")
    ...
    def _on_apply_attack_clicked(self):
        ...
        try:
            ...
            if not hasattr(self, 'original_secret'):
                self.lbl_ber.setText("BER：——（未嵌入过信息，无法计算）")
            else:
                self.original_secret = self.txt_secret.toPlainText()

            # 更新BER曲线
            #self._update_ber_chart(attack_type, param_value)

            self.status_label.setText(f"{attack_type}攻击应用成功")
        except Exception as e:
            QMessageBox.critical(self, "错误", f"应用攻击失败: {str(e)}。请检查攻击算法和输入参数是否正确。")
    ...
    def _update_metrics(self, original_image, modified_image):
        ...
        try:
            ...
            QMessageBox.warning(self, "警告", f"计算评估指标时出错: {str(e)}。请检查输入图像数据是否正确。")
    ...
    def _update_ber_chart(self, attack_type, param_value):
        ...
        try:
            ...
            print(f"更新BER曲线时出错: {str(e)}。请检查提取算法和输入数据是否正确。")
